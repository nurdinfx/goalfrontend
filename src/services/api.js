...
export const apiService = {
  ...
  getVillages: async () => {
    try {
      ...
      const data = toArray(handleResponse(response));
      try {
      ...
      try {
        const raw = localStorage.getItem('villages');
    ...
    try {
      ...
      if (error.response?.status === 404) {
        try {
          ...
          const name = String(villageData?.name || '').trim();
          const code = ('VIL-' + name.toUpperCase().replace(/[^A-Z0-9]+/g, '-').replace(/^-+|-+$/g, '')).slice(0,24);
          const item = { _id: 'local-' + Date.now(), name, code, location: 'Unknown', monthlyFee: 0, status: 'active', createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() };
          const next = [item, ...arr];
          localStorage.setItem('villages', JSON.stringify(next));
          return item;
        } catch {
      ...
      if (error.response?.status === 404) {
        try {
          ...
          if (idx !== -1) {
            ...
            arr[idx] = updated;
            localStorage.setItem('villages', JSON.stringify(arr));
            return updated;
          }
