...
const Payments = ({ villages, customers, updateCustomer }) => {
  ...
  const handleMarkPaid = async (customerId) => {
    setActionLoading(`paid-${customerId}`);
  ...
  const handleMarkUnpaid = async (customerId) => {
    setActionLoading(`unpaid-${customerId}`);
  ...
  const handlePartialPayment = async (customerId, amount) => {
    setActionLoading(`partial-${customerId}`);
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {processedPayments.map((payment) => {
                const isPaidLoading = actionLoading === `paid-${payment.id}`;
                const isUnpaidLoading = actionLoading === `unpaid-${payment.id}`;
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium space-x-2">
                      {payment.status === 'unpaid' && (
                        <button
                          ...
                          className="bg-green-600 text-white px-3 py-1 rounded text-xs hover:bg-green-700 disabled:opacity-50 transition duration-200 flex items-center"
                        >
                          {isPaidLoading ? (
                            <>
                      ...
                      {payment.status === 'paid' && (
                        <>
                          ...
                          <button
                            ...
                            disabled={isUnpaidLoading}
                            className="bg-red-600 text-white px-3 py-1 rounded text-xs hover:bg-red-700 disabled:opacity-50 transition duration-200 flex items-center"
                          >
                            {isUnpaidLoading ? (
                              <>
                                <div className="w-3 h-3 border-2 border-white border-t-transparent rounded-full animate-spin mr-1"></div>
